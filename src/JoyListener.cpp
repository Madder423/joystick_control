...
JoyListener::JoyListener(std::string name,std::string _sub_topic_name): Node(name), sub_topic_name(_sub_topic_name){
    ...
    //创建订阅者
    subscribe = this->create_subscription<bupt_interfaces::msg::Joy>(sub_topic_name,10,std::bind(&JoyListener::sub_callback,this,std::placeholders::_1));
}
...
void JoyListener::sub_callback(const bupt_interfaces::msg::Joy::SharedPtr msg){
